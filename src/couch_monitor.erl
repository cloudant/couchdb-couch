...
cancel(Mod, Name, {Client, Monitor, IsSys})
    ...
    case (catch ets:update_counter(Mod:tab_name(counters), Name, -1)) of
        0 when not IsSys ->
            true = couch_monitor_server:insert(Mod, idle, Name, ok),
            ok;
        _ ->
            ok
    end;
        ...
        undefined ->
            ...
            case khash:size(St#st.client_refs) of
                0 ->
                    ...
                    couch_monitor_server:delete(St#st.mod, idle, St#st.name);
...
maybe_set_idle(St) ->
    case khash:size(St#st.client_refs) of
        ...
        0 ->
            % We're now idle
            couch_monitor_server:insert(St#st.mod, idle, St#st.name, {});
        N when is_integer(N), N > 0 ->
