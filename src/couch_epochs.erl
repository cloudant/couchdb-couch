...
update(#couch_epochs{epochs = Epochs}, UpdateSeq) ->
...
upgrade_epochs_test() ->
    ...
    NewEpochs = #couch_epochs{epochs = TakenOver} = update(OldEpochs, 20),
    ?assertEqual(TakeOverExpect, TakenOver),

    % If we update again we don't add a new
    % epoch record or change the update seq.
    #couch_epochs{epochs = SameEpochs} = update(NewEpochs, 30),
    ?assertEqual(TakeOverExpect, SameEpochs).
