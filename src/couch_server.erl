...
terminate(Reason, Srv) ->
    ...
    ets:foldl(fun(#db{main_pid=Pid}, _) ->
...
maybe_close_idle_db(#server{dbs_open=NumOpen, max_dbs_open=MaxOpen}=Server)
...
open_async(Server, From, DbName, Filepath, Options) ->
    ...
    {ok, MonPid} = couch_server_monitor:start_link(DbName, From, IsSysDb),
    Monitor = #mon{name = DbName, pid = MonPid},
    true = ets:insert(?DBS, #db{
        name = DbName,
        fd = ReqType,
        main_pid = Opener,
        compactor_pid = Monitor,
        fd_monitor = locked,
        options = Options
    }),
...
handle_call({open_result, T0, DbName, {ok, Db}}, {FromPid, _Tag}, Server) ->
    ...
    % - compactor_pid is the monitor
    % - fd used to possibly store a creation request
    [#db{
        fd = ReqType,
        compactor_pid = #mon{pid = MonPid}
    }] = ets:lookup(?DBS, DbName),
    gen_server:cast(MonPid, {reply_ok, T0, {ok, Db}}),
...
handle_call({open_result, _T0, DbName, Error}, {FromPid, _Tag}, Server) ->
    ...
    gen_server:cast(MonPid, {reply_error, Error}),
    true = ets:delete(?DBS, DbName),
    true = ets:delete(?PIDS, FromPid),
    true = ets:delete(?MONITORS, DbName),
    true = ets:delete(?IDLE, DbName),
...
handle_call({open, DbName, Options}, From, Server) ->
    ...
    [#db{compactor_pid = #mon{pid = MonPid}}] ->
        % icky hack of field values
        % - compactor_pid used to store monitor
        gen_server:cast(MonPid, {add_waiter, From}),
        {noreply, Server};
    [#db{} = Db] ->
        {reply, {ok, Db}, Server}
    end;
...
handle_call({delete, DbName, Options}, _From, Server) ->
    ...
    ok ->
        ...
        case ets:lookup(?DBS, DbName) of
        [] -> Server;
        [#db{compactor_pid = #mon{pid = MonPid}} = Db] ->
            % icky hack of field values
            % - compactor_pid used to store monitor
            true = ets:delete(?DBS, DbName),
            ...
            true = ets:delete(?MONITORS, DbName),
            true = ets:delete(?IDLE, DbName),
            gen_server:cast(MonPid, {reply_error, not_found}),
...
handle_call({db_updated, #db{}=Db}, _From, Server) ->
    ...
    case (catch ets:lookup_element(?DBS, DbName, #db.instance_start_time)) of
        StartTime ->
            true = ets:insert(?DBS, Db);
        _ ->
            % Don't freak out here because we could
            % be processing old messages from db
            % updaters that have since died.
            ok
    end,
    {reply, ok, Server}.
...
handle_info({'EXIT', Pid, Reason}, Server) ->
    ...
    [{Pid, DbName}] ->
        [#db{compactor_pid = Monitor} = Db] = ets:lookup(?DBS, DbName),
        if Reason /= snappy_nif_not_loaded -> ok; true ->
            Msg = io_lib:format("To open the database `~s`, Apache CouchDB "
                "must be built with Erlang OTP R13B04 or higher.", [DbName]),
            couch_log:error(Msg, [])
        end,
        couch_log:info("db ~s died with reason ~r", [DbName, Reason]),
        % icky hack of field values
        % - compactor_pid used to store monitor
        case Monitor of
            #mon{pid = MonPid} ->
                gen_server:cast(MonPid, {reply_error, Reason});
            _ ->
                ok
        end,
        true = ets:delete(?DBS, DbName),
...
incref(DbName) ->
    gen_server:cast(?MODULE, {incref, DbName}).
